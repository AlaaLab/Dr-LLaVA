...
def eval_model(args):
    ...
    for line_chunks in tqdm(questions):
        ...
        _ls = [(x,y) for (x,y) in zip(k,line_chunks)]
        random.shuffle(_ls)
        line_chunks = _ls.copy()

        # random a number between 2 to 5

        def random_number_between_2_and_5():
            # Generate a random number between 2 and 5
            return random.randint(2, 5)
        line_chunks  = random.sample(line_chunks, random_number_between_2_and_5())
        conv = conv_templates[args.conv_mode].copy()
        
        for (k,line) in line_chunks:
