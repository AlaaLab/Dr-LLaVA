...
sys.path.append('/wynton/protected/group/ibrahim/harry/LLaVA_checkpoints/LLaVA-RLHF/Eval')
from text_generation.generate_prompts import random_conterfactual_prior
...
def eval_model(args):
    ...
    for line in tqdm(questions):
        if p%5 ==0:
            # meaning this is the first question with no prior information that can be persued
            p+=1
            continue
        ...
        qs = line["text"]
        prior_sentence= qs.split('. ')[0]+'.'
        prior_prompt = qs.split(': ')[0] +': '
        prior_claim = random_conterfactual_prior(category, p%5)
        cur_prompt = qs.split('. ')[-1]

        cur_prompt = prior_prompt + prior_claim +' '+ cur_prompt
        p+=1
