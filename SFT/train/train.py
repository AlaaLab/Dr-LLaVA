...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # print(i, sources, data_dict['input_ids'].shape, data_dict['labels'].shape)
        # image exist in the data
        if "image" in self.list_data_dict[i]:
            data_dict["image"] = image.to(self.data_args.dtype)
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict["image"] = torch.zeros(
                3, crop_size["height"], crop_size["width"]
            ).to(self.data_args.dtype)
            # i = random.randint( 0, len(self.list_data_dict) )
            # return self.__getitem__(i)
        return data_dict


class LazyM3ITSupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(
        ...
        data_list = [
            ...
            "mocheg",
            "vsr",
            "refcoco",  # classfication
            "science-qa",  # reasoning
            "vqa-v2",
            "gqa",
            "st-vqa",
            "text-vqa",
        ...
        list_data_dict = concatenate_datasets(
            [load_dataset("MMInstruction/M3IT", d)["train"] for d in data_list]
        )
        ...
        self.list_data_dict = list_data_dict
        self.data_args = data_args
        self.llava_list_data_dict = None
        if data_path is not None and data_path.endswith(".json"):
            llava_list_data_dict = json.load(open(data_path, "r"))
            self.llava_list_data_dict = llava_list_data_dict
        self.llava_data_len = (
            len(self.llava_list_data_dict)
            if self.llava_list_data_dict is not None
            else 0
        )
        self.m3it_data_len = self.llava_data_len + len(self.list_data_dict)

    def __len__(self):
        return self.m3it_data_len

    def __llava_getitem__(self, i) -> Dict[str, torch.Tensor]:
        sources = self.llava_list_data_dict[i]
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        if "image" in sources[0]:
            ...
            sources = copy.deepcopy([e["conversations"] for e in sources])
        data_dict = preprocess(
            sources, self.tokenizer, has_image=("image" in self.llava_list_data_dict[i])
        )
        if isinstance(i, int):
            data_dict = dict(
                input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0]
        ...
        # image exist in the data
        if "image" in self.llava_list_data_dict[i]:
            data_dict["image"] = image.to(self.data_args.dtype)
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        if i < self.llava_data_len:
            return self.__llava_getitem__(i)
        i -= self.llava_data_len
        train_instance = self.list_data_dict[i]
        instruction = train_instance["instruction"]  # str
        inputs = train_instance["inputs"]  # str
        outputs = train_instance["outputs"]
        # sources = self.list_data_dict[i]
        image_base64_str_list = train_instance["image_base64_str"]
        has_image = image_base64_str_list[0] is not None

        while image_base64_str_list[0] is None:
            import random

            i = random.randint(0, len(self.list_data_dict) + self.llava_data_len)
            if i < self.llava_data_len:
                return self.__llava_getitem__(i)
            i -= self.llava_data_len
            train_instance = self.list_data_dict[i]
            ...
            # sources = self.list_data_dict[i]
            image_base64_str_list = train_instance["image_base64_str"]
            print(f"index change to {i}, image is None")
        # formuate m3it data into llava format
        if not len(inputs):
        ...
        while c_channel != 3:
            ...
            # sources = self.list_data_dict[i]
            image_base64_str_list = train_instance["image_base64_str"]
            print(f"index change to {i}, num_channel is {c_channel} previously.")
            if image_base64_str_list[0] is None:
                continue
            print(f"index change to {i}, num_channel is {c_channel} previously.")
            image = Image.open(BytesIO(b64decode(image_base64_str_list[0]))).convert(
                "RGB"
            )
            i_numpy = to_numpy_array(image)
            c_axis = get_channel_dimension_axis(i_numpy)
        ...
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        if "image_base64_str" in train_instance:
            processor = self.data_args.image_processor
            if self.data_args.image_aspect_ratio == "keep":
...
def make_supervised_data_module(
    ...
    if data_args.mimicit_dataset:
        # from mimicit_data_llava import MimicitDataset
        train_dataset = LazyM3ITSupervisedDataset(
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        assert vision_tower.dtype == compute_dtype
        data_args.dtype = compute_dtype
        ...
        if training_args.bits in [4, 8]:
            model.get_model().mm_projector.to(
                dtype=compute_dtype, device=current_device
            )
